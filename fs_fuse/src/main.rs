...
impl BlockDevice for BlockFile {
    fn read_block(&self, block_id: usize, buf: &mut [u8]) {
        ...
        // let flag = file.read(buf).unwrap();
        // println!("{}", flag);
        assert_eq!(file.read(buf).unwrap(), BLOCK_SZ, "not as complete block!")
    }
...
fn fs_pack() -> std::io::Result<()> {
    let matches = App::new("file_system packer")
        .arg(
